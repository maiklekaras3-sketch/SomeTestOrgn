...
func TestClientV1ZoneZoneIDGet(t *testing.T) {
	...
	_, err := client.Client.V1.Zones.ZoneID.Get(context.TODO(), stnlsstest.ClientV1ZoneZoneIDGetParams{
...
func TestClientV1ZoneZoneIDDelete(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Client.V1.Zones.ZoneID.Delete(context.TODO(), stnlsstest.ClientV1ZoneZoneIDDeleteParams{
		XAuthEmail: "X-Auth-Email",
		XAuthKey:   "X-Auth-Key",
	})
	if err != nil {
